/**
 * @file EsportatoreTxt.java
 * File contenente la classe EsportatoreTxt per gestire l'esportazione in formato txt
 */
...
public class EsportatoreTxt extends Esportatore {
	 ...
	 */
	public EsportatoreTxt(String nomefile, ElencoVoci elencoVoci) {
		super(nomefile, elencoVoci);
	}

	/**
	 * Implementazione del metodo esporta() di {@link Esportatore} 
	 * Esporta in formato txt l'oggetto {@link ElencoVoci} nel file 'nomefile'
	 */
	@Override
	public void esporta() {
		...
		try {
			...
			for(int i=0; i<super.getElencoVoci().length(); i++) {
				fw.write(super.getElencoVoci().get(i).toTxt() + "\n");
			}
